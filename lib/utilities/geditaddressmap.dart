...
class GeditAddressMap extends StatefulWidget {
  GeditAddressMap({Key? key, required this.latitudevalue,required this.longitudevalue}) : super(key: key);
  String latitudevalue;
  String longitudevalue;
  @override
  State<GeditAddressMap> createState() => _GeditAddressMapState();
}

class _GeditAddressMapState extends State<GeditAddressMap> {
               ...
               child: InkWell(
                 onTap: () async {
                  var place = await PlacesAutocomplete.show(
                          ...
                          mode: Mode.overlay,
                          language: "in",
                          logo: const Text('',),//Text("Powered by Veramasa",textAlign: TextAlign.center,),//Text("Powered by Veramasa",textAlign: TextAlign.center,),
                          types: [],
                          strictbounds: false,
  ...
  void _getCurrentLocation() async{
    double lat = double.parse(widget.latitudevalue);
    ...
    List<Placemark> placemarks = await placemarkFromCoordinates(lat, lng);
        ...
        Util.addStringToSF('longvalue', longString, '');*/
        final GoogleMapController mapController = await _controller.future;
        mapController.animateCamera(CameraUpdate.newCameraPosition(CameraPosition(target: LatLng(lat, lng), zoom:20)));
        setState(() {});
  }
    ...
    try {
      if (response.statusCode == 200) {
        ...
        if (dec['success'] == true) {
          ...
          /*Navigator.push(context,MaterialPageRoute(builder: (context) =>  AddressForm(type: 'edit',
                                  state: 0,
                                  address: const [],
          )));*/
        } else {
